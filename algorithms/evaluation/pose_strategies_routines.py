...
def align_objects_by_axis(object_axis, target_axis, rotation_angle, translation_distance):
    """
    Calcule la transformation à appliquer à l'objet 2 pour l'aligner sur l'objet 1
    selon la méthode PCA-based alignment.
    """

    # calculer la matrice de rotation pour aligner les deux axes
    R_align = get_rotation_matrix_from_vectors(axis1=object_axis, axis2=target_axis)

    aligned_axis = R_align @ object_axis

    # Appliquer une rotation autour de l’axe aligné (genome_rotation)
    R_rotation = get_rotation_matrix_from_an_axis_and_an_angle(direction_vector=aligned_axis, theta=rotation_angle)

    #composer deux rotation successives : align apres rotaion autour de l'axe
    R_total = R_rotation @ R_align

    T = translation_distance * target_axis / np.linalg.norm(target_axis)

    return R_total, T # Rotation (3x3) et translation (3,)
...
def get_rotation_matrix_from_vectors(axis1, axis2):
    if np.all(axis1 == [0, 0, 0]) or np.all(axis2 == [0, 0, 0]):
        raise ValueError("L’un des axes est nul : impossible de normaliser ou construire une rotation.")

    # Calculer la rotation pour aligner axis2 sur axis1
    axis1 = normalize_vector(axis1)
    axis2 = normalize_vector(axis2)

    direction_vector = np.cross(axis2, axis1)
    s = np.linalg.norm(direction_vector)   # this gives sin(angle de rotation)
    c = np.dot(axis2, axis1)    # this gives cos(angle de rotation)

    if s < 1e-6:  # Les vecteurs sont déjà alignés
        return np.eye(3)
...
def display_pca_axis_debug(bullet_client, pca_axes, origin, scale=0.1):

    colors = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]  # X (red), Y (green), Z (blue)

    for i, axis in enumerate(pca_axes):
        end_point = (np.array(origin) + axis * scale).tolist()
        bullet_client.addUserDebugLine(
            lineFromXYZ=origin,
            lineToXYZ=end_point,
            lineColorRGB=colors[i],
            lineWidth=2.0,
            lifeTime=0  # 0 means infinite duration
        )
