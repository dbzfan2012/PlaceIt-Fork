...
class UnstructuredArchive(Archive):
    ...
    def __init__(self, archive_limit_size, archive_limit_strat, pop_size, fill_archive_strat):
    ...
    def _compute_archive_nb2add_per_update(self, pop_size):
        ...
        n_inds2add_from_pop_pb = PB_ADD2ARCHIVE * pop_size
        n_inds2add = int(n_inds2add_from_pop_pb * qd_cfg.OFFSPRING_NB_COEFF)
    ...
    def _apply_removal_strategy(self, archive_limit_strat):
        ...
        nb_ind_to_keep = int(original_len * qd_cfg.ARCHIVE_DECREMENTAL_RATIO)
        nb_ind_to_remove = original_len - nb_ind_to_keep

        # removal strategies
        if archive_limit_strat == qd_cfg.ArchiveLimitStrategy.RANDOM:
            self._apply_removal_strategy_random(nb_ind_to_keep=nb_ind_to_keep)
        else:
            raise NotImplementedError(f'{archive_limit_strat} not supported.')
