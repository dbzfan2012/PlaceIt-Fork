...
class Timer:
    ...
    def get_all(self, format='seconds'):
        ...
        if format not in SUPPORTED_TIMER_FORMATS:
            raise AttributeError(f'not supported timer format: {format} (supported: {SUPPORTED_TIMER_FORMATS}')
...
def is_running_timeout(timer, label):
    elapsed_time = timer.get_on_the_fly_time(label)
    return elapsed_time > ex_cfg.MAX_RUN_TIME_IN_S
