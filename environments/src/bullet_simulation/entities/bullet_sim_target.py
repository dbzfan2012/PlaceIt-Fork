...
ENV_LATERAL_FRICTION_VALUE = 0.3

class BulletSimTarget:
    def __init__(self, bullet_client, name, use_concave_mesh):

        self.target_name = None  # striped object name
        self.target_id = None  # id associated with the object in the bullet simulation
        ...
        self.use_concave_mesh=use_concave_mesh,
    ...
    def _init_attributes(self, bullet_client, name):
        ...
        if name is None:
            print('Warning : no given object for environment.')
            return

        self.target_name = self._init_env_name(name)
        ...
        self._search_space_bb_object = get_search_space_bb_object(obj_id=self.target_id)
        ...
        # inertial_pose = self.extract_inertial_pose_from_urdf(self.target_name)
        # display_pca_axis_debug(bullet_client=bullet_client, pca_axes=self._pca_axes, origin=inertial_pose)
    ...
    def _init_env_name(self, object_name):
    ...
    def load_object(self, bullet_client, object_name=None):
        ...
        use_concave_mesh=self.use_concave_mesh

        if use_concave_mesh:
            path2mesh = self.extract_path2obj_contact_point_cloud_from_urdf(object_name)
            ...
            env_obj = bullet_client.createMultiBody(
                baseMass=0,
            ...
            try:
                env_obj = bullet_client.loadURDF(
                    fileName=str(urdf),
                    basePosition=DEFAULT_OBJECT_POSE,
                    baseOrientation=DEFAULT_OBJECT_ORIENT,
                    useFixedBase=True,
        ...
        bullet_client.changeDynamics(
            bodyUniqueId=env_obj,
            linkIndex=-1,
            lateralFriction=ENV_LATERAL_FRICTION_VALUE,
        )

        # mug_tree and cube : rgbaColor=[0.8, 0.6, 0.4, 1]
        # bowl : rgbaColor=[0.44, 0.11, 0.11, 1.0]
        # table : rgbaColor=[0.66, 0.49, 0.32, 1.0]

        bullet_client.changeVisualShape(env_obj, -1, rgbaColor=[0.66, 0.49, 0.32, 1.0]) 
        self.target_id = env_obj


    def update_infos(self, bullet_client, info):
        is_obj_initialized = self.target_id is not None
